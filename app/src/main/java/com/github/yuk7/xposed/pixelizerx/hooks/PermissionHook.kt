...
class PermissionHook: IXposedHookLoadPackage {
    override fun handleLoadPackage(lpparam: XC_LoadPackage.LoadPackageParam?) {
        ...
        XposedHelpers.findAndHookMethod(Application::class.java, "attach", Context::class.java, object:
            XC_MethodHook() {
    ...
    private fun hookMain(lpparam: XC_LoadPackage.LoadPackageParam, allowList: List<String>, denyList: List<String>) {
        XposedBridge.log("permission hooking:" + lpparam.packageName)
        XposedHelpers.findAndHookMethod(applicationManager, lpparam.classLoader,"hasSystemFeature", String::class.java, Int::class.java, object: XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam?) {
                super.beforeHookedMethod(param)
                hookedHasSystemFeature(param, allowList, denyList)
            }
        })
        XposedHelpers.findAndHookMethod(applicationManager, lpparam.classLoader,"hasSystemFeature", String::class.java, object: XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam?) {
                super.beforeHookedMethod(param)
                hookedHasSystemFeature(param, allowList, denyList)
            }
    ...
    private fun hookedHasSystemFeature(param: XC_MethodHook.MethodHookParam?, allowList: List<String>, denyList: List<String>) {
        if (param == null) {
            return
        }
        param.args.forEach {
